...
void Flash_ReadChar(uint8_t* dataOut, uint32_t addr1, uint32_t addr2)
{
	...
	if((uint8_t)Flash_Read_Int(addr2+(uint32_t)2) == 255)
	{
		check = (uint8_t)Flash_Read_Int(addr2)-48;
	}
	else
	{
		check = ((uint8_t)Flash_Read_Int(addr2)-48)*10 + (uint8_t)Flash_Read_Int(addr2+2)-48;
	}
