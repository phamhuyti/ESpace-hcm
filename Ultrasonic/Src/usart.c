...
void serial_write(int port, uint8_t *text, uint16_t size)
{
  if (port == 1)
  {
    HAL_UART_Transmit_DMA(&huart1, text, size); //check strlen variables main.h
  }
  else
  {
...
uint8_t serial_Available(int uart)
{
  if (uart == 1)
  {
    return !((hdma_usart1_rx.Lock) && (hdma_usart1_tx.Lock));
  }
  else
  {
    return !((hdma_usart2_rx.Lock) && (hdma_usart2_tx.Lock));
  }
}
